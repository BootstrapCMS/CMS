        ...
        <div class="well clearfix">
            @if (Sentry::check() && Sentry::getUser()->hasAccess('mod'))
                <div class="col-md-9 col-sm-8 col-xs-12">
                    ...
                    <p>
                        @if (Sentry::check() && Sentry::getUser()->hasAccess('mod'))
                            <a href="#" id="editable_comment_{{ $comment->getId() }}" class="x-editable" data-type="text" data-pk="{{ $comment->getId() }}" data-url="{{ URL::route('blog.posts.comments.update', array('posts' => $post->getId(), 'comments' => $comment->getId())) }}" data-ajaxOptions="{type: "patch"}" data-title="Modify comment">{{ nl2br(e($comment->getBody())) }}</a>
                        @else
