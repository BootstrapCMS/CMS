<div class="well clearfix">
    @if (Sentry::check() && Sentry::getUser()->hasAccess('mod'))
        <div class="col-md-10 col-sm-9">
            ...
            <p>
                <a href="#" id="editable_comment_{{ $comment->getId() }}" class="x-editable" data-type="textarea" data-inputclass="form-control comment-box col-sm-8" data-placeholder="Type a comment..." data-rows="3" data-pk="{{ $comment->getId() }}" data-url="{{ URL::route('blog.posts.comments.update', array('posts' => $comment->getPostId(), 'comments' => $comment->getId())) }}" data-title="Modify comment">{{ nl2br(e($comment->getBody())) }}</a>
            </p>
        </div>
        <div class="hidden-xs">
