<form class="form-horizontal" action="{{ $form['url'] }}" method="post">   
    ...
    <div class="control-group {{ ($errors->has('title')) ? 'error' : '' }}" for="title">
        ...
        <div class="controls">
            <input name="title" value="{{ Request::old('title', $form['defaults']['title']) }}" type="text" class="input-xlarge" placeholder="Page Title">
    ...
    <div class="control-group {{ ($errors->has('icon')) ? 'error' : '' }}" for="icon">
        ...
        <div class="controls">
            <input name="icon" value="{{ Request::old('icon', $form['defaults']['icon']) }}" type="text" class="input-xlarge" placeholder="Page Icon">
    ...
    <div class="control-group {{ ($errors->has('body')) ? 'error' : '' }}" for="body">
        ...
        <div class="controls">
            <textarea name="body" type="text" class="input-xlarge" placeholder="Page Body" rows="8">{{ Request::old('body', $form['defaults']['body']) }}</textarea>
    ...
    <div class="control-group {{ ($errors->has('show_title')) ? 'error' : '' }}" for="show_title">
        ...
        <div class="controls">
            <div class="switch" data-on-label="<i class='icon-ok icon-white'></i>" data-off-label="<i class='icon-remove'>">
                <input name="show_title" type="checkbox" {{ (Request::old('show_title', $form['defaults']['show_title']) == true) ? 'checked' : '' }} >
            </div>
    ...
    <div class="control-group {{ ($errors->has('show_nav')) ? 'error' : '' }}" for="show_nav">
        ...
        <div class="controls">
            <div class="switch" data-on-label="<i class='icon-ok icon-white'></i>" data-off-label="<i class='icon-remove'>">
                <input name="show_nav" type="checkbox" {{ (Request::old('show_nav', $form['defaults']['show_nav']) == true) ? 'checked' : '' }} >
            </div>
