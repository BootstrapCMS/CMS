...
function cmsCommentCreateSuccess(data, status, xhr) {
    if (!xhr.responseJSON) {
...
function cmsCommentCreateError(xhr, status, error) {
...
function cmsCommentCreateSubmit(that) {
    ...
    $(that).ajaxSubmit({ 
        ...
        resetForm: true,
        timeout: 5000,
        success: cmsCommentCreateSuccess(data, status, xhr),
        error: cmsCommentCreateError(xhr, status, error)
    });
}
